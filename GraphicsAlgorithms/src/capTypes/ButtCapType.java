...
public class ButtCapType implements CapType {

	@Override
	public void drawCap(Line line, Point center, Graphics g) {
		Point point1 = line.getP1();
		Point point2 = line.getP2();
		Point p1, p2;
		int thickness = line.getThickness();
		if (point1.x == point2.x) {// Infinite slope
			...
			if(point1.x < point2.x){
				...
				p1 = new Point(point2.x, point2.y);
			...
			if (slope <= 1 && slope >= -1) {
				...
				Point t4 = new Point();
				t4.x = p2.x - thickness/2 + (int) Math.round(thickness * cos * cos );
